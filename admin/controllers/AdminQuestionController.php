...
class AdminQuestionController {
    ...
    public function index() {
            ...
            try {
                ...
                $query = "
                    SELECT q.*, e.title as exam_title, c.title as course_title,
                           a.name as creator_name, a.username as creator_username
                    FROM questions q
                ...
                if ($exam_id == 0) {
                    ...
                    ");
                    $stmt->execute();
                    $questionCounts = $stmt->fetchAll(PDO::FETCH_KEY_PAIR);
                }
        ...
        if ($conn) {
            try {
                ...
                if (isset($questionCounts) && !empty($questionCounts)) {
                    foreach ($exams as &$exam_item) {
                        $exam_item['question_count'] = isset($questionCounts[$exam_item['id']]) ? $questionCounts[$exam_item['id']] : 0;
                    }
                    unset($exam_item);
                }
            } catch (PDOException $e) {
    ...
    public function store() {
        ...
        if (isset($_FILES['question_image']) && $_FILES['question_image']['error'] === UPLOAD_ERR_OK) {
            ...
            if (in_array($file_ext, $allowed_exts)) {
                ...
                $new_file_name = 'question_' . time() . '_' . uniqid() . '.' . $file_ext;
                $upload_path = $upload_dir . $new_file_name;
                
                // Move uploaded file
                if (move_uploaded_file($file_tmp, $upload_path)) {
                    $image_path = '/uploads/questions/' . $new_file_name;
                }
            }
        }
        ...
        if ($conn && empty($errors)) {
            try {
                $stmt = $conn->prepare("
                    INSERT INTO questions (exam_id, question_text, image_path, question_type, options, correct_answer, points, admin_id, created_at, updated_at)
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?, NOW(), NOW())
                ");
                ...
                $logger->log(
                    ...
                    [
                        ...
                        'question_text' => mb_substr($question_text, 0, 100) . (mb_strlen($question_text) > 100 ? '...' : '')
                    ]
                );
    ...
    public function view($id) {
        ...
        // Set page title
        $pageTitle = 'عرض السؤال: ' . htmlspecialchars(substr($question['question_text'], 0, 50)) . (strlen($question['question_text']) > 50 ? '...' : '');
    ...
    public function update($id) {
        ...
        if ($conn && empty($errors)) {
            try {
                // Get current question data if we need to keep the existing image
                if ($keep_existing_image) {
                    $stmt = $conn->prepare("SELECT image_path FROM questions WHERE id = ?");
                    $stmt->execute([$id]);
                    $current_question = $stmt->fetch(PDO::FETCH_ASSOC);
                    if ($current_question && !empty($current_question['image_path'])) {
                ...
                $stmt = $conn->prepare("
                    UPDATE questions
                    SET exam_id = ?, question_text = ?, image_path = ?, question_type = ?, options = ?, correct_answer = ?, points = ?, updated_at = NOW()
                    WHERE id = ?
                ");
    ...
    public function delete($id) {
        ...
        if ($conn) {
            try {
                ...
                $questionStmt = $conn->prepare("SELECT question_text, exam_id FROM questions WHERE id = ?");
                ...
                $logger->log(
                    ...
                    [
                        ...
                        'question_text' => mb_substr($question['question_text'] ?? 'unknown', 0, 100) . (mb_strlen($question['question_text'] ?? '') > 100 ? '...' : '')
                    ]
                );
